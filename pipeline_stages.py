...
"""
def run_inter_sequence_feature_stage():

    # Adds inter-sequence features to the pre-engineered feature files.
    # This stage loads all per-sequence features, calculates subject-level stats,
    # adds deviation features, and overwrites the files.

    if not config.ENABLE_INTER_SEQUENCE_FEATURES:
        print("\n" + "="*80 + "\n--- STAGE 3: Inter-Sequence Features ---\n" + "="*80)
        print("Skipping inter-sequence feature generation as config.ENABLE_INTER_SEQUENCE_FEATURES is False.")
        return

    print("\n" + "="*80 + "\n--- STAGE 3: Inter-Sequence Feature Generation ---\n" + "="*80)

    # Checkpoint: If stats file exists and not forced, skip.
    if os.path.exists(config.SUBJECT_STATS_PATH) and not config.FORCE_RERUN_INTER_SEQUENCE:
        print("Inter-sequence features already generated. Skipping.")
        return

    print("Loading all per-sequence feature files to add inter-sequence features...")
    ...
    if not all_feature_files:
        raise FileNotFoundError("No pre-engineered feature files found. Run Stage 2 first.")

    df_all_features = pl.read_parquet(all_feature_files)

    print("Calculating subject-level statistics...")
    static_feature_cols = [col for col in df_all_features.columns if '_mean' in col or '_std' in col or 'fft_amp' in col]

    subject_means_stds = df_all_features.group_by('subject').agg(
        [pl.col(col).mean().alias(f"{col}_subject_mean") for col in static_feature_cols] +
        [pl.col(col).std().alias(f"{col}_subject_std").fill_null(1e-9) for col in static_feature_cols]
    )

    print("Joining subject stats and calculating deviation features...")
    df_all_features = df_all_features.join(subject_means_stds, on='subject', how='left')

    deviation_expressions = []
    for col in static_feature_cols:
        deviation_expressions.append(
            (pl.col(col) - pl.col(f"{col}_subject_mean")).alias(f'{col}_dev_from_subj_mean')
        )
        deviation_expressions.append(
            (pl.col(col) / pl.col(f"{col}_subject_std")).alias(f'{col}_dev_from_subj_std')
    ...
    df_all_features = df_all_features.with_columns(deviation_expressions)
    df_all_features = df_all_features.with_columns(
        pl.col('sequence_id').rank(method='dense').over('subject').alias('sequence_position_in_subject')
    )

    # Drop temporary subject mean/std columns
    cols_to_drop = [col for col in df_all_features.columns if col.endswith('_subject_mean') or col.endswith('_subject_std')]
    df_all_features = df_all_features.drop(cols_to_drop)
    df_all_features = df_all_features.fill_nan(0.0).fill_null(0.0)

    print("Overwriting feature files with inter-sequence features...")
    # This is memory intensive but necessary for this feature type.
    # It overwrites the original files with the enriched data.
    for seq_id, group_df in df_all_features.group_by("sequence_id"):
        output_path = os.path.join(config.PERMANENT_FEATURE_DIR, f"seq_{seq_id}_features.parquet")
        group_df.write_parquet(output_path)

    joblib.dump(subject_means_stds, config.SUBJECT_STATS_PATH)
    print(f"Subject-level statistics saved to {config.SUBJECT_STATS_PATH}")
    print("Inter-sequence feature stage complete.")
    del df_all_features, subject_means_stds
    gc.collect()
"""
